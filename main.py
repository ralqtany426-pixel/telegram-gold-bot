...
# --- منطق التحليل الديناميكي الحقيقي (شراء وبيع) ---
            ...
            if users:
                ...
                for chat_id in users:
                    try:
                        ...
                        pass
                time.sleep(7200) # تنبيه كل ساعتين لمنع الإزعاج
            time.sleep(60)
        except:
            time.sleep(60)
...
@app.route('/')
def home():
    return "Bot is active and running with Dynamic Signals!", 200
...
def start_command(message):
    ...
    welcome_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"مرحباً بك مجدداً يا عبد الله. تم تفعيل خوارزميات الشراء والبيع الديناميكية بنجاح.\n"
...
def handle_text_messages(message):
    ...
    add_user(message.chat.id)
    price = get_gold_price()
    action_title, order_type, zone_entree, stop_loss, tp1, tp2, tp3, bias, is_buy = get_institutional_signal(price)
    ...
    if text == "💰 السعر اللحظي":
        ...
        bot.send_message(message.chat.id, 
            ...
            f"🧱 نطاق السيولة الفعّال: `{zone_entree}`\n"
            f"💡 **القرار الفني:** `البحث عن الفرص المتوافقة مع اتجاه السيولة الحالي.`", 
            parse_mode="Markdown")
        ...
        if new_status == 1:
            bot.send_message(message.chat.id, "🟢 **تم تفعيل تنبيهات الصفقات بنجاح!**", parse_mode="Markdown")
        else:
            bot.send_message(message.chat.id, "🔴 **تم إيقاف التنبيهات مؤقتاً.**", parse_mode="Markdown")
